...
describe("ClickService", () => {
  ...
  describe("create", () => {
    test("creates a new click record", async () => {
      ...
      expect(db.insert).toHaveBeenCalledWith(schema.click);
  ...
  describe("list", () => {
    test("returns paginated list of clicks", async () => {
      ...
      expect(selectMock.from).toHaveBeenCalledWith(schema.click);
  ...
  describe("getStats", () => {
    test("returns comprehensive click statistics", async () => {
      ...
      const topReferrersMock = {
        ...
        limit: vi
          .fn()
          .mockResolvedValue([{ referrerDomain: "google.com", clicks: 30 }]),
      };
    ...
    test("applies date filters when provided", async () => {
      ...
      const result = await ClickService.getStats(
        "url123456",
        startDate,
        endDate,
      );
  ...
  describe("deleteById", () => {
    test("deletes a click record successfully", async () => {
      ...
      expect(db.delete).toHaveBeenCalledWith(schema.click);
  ...
  describe("deleteByUrlId", () => {
    test("deletes all clicks for a URL", async () => {
      ...
      expect(db.delete).toHaveBeenCalledWith(schema.click);
      expect(deleteMock.where).toHaveBeenCalledWith(
        eq(schema.click.urlId, "url123456"),
      );
    });
  });
