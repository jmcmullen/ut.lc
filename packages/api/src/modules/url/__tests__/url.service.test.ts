...
describe("UrlService", () => {
  ...
  describe("list", () => {
    test("returns paginated list of URLs", async () => {
      ...
      expect(selectMock.from).toHaveBeenCalledWith(schema.url);
      expect(selectMock.where).toHaveBeenCalledWith(
        eq(schema.url.userId, mockUserId),
      );
  ...
  describe("create", () => {
    test("creates a new URL without custom ID", async () => {
      ...
      expect(db.insert).toHaveBeenCalledWith(schema.url);
  ...
  describe("remove", () => {
    test("deletes an existing URL", async () => {
      ...
      expect(result).toEqual(mockUrl);
      expect(db.delete).toHaveBeenCalledWith(schema.url);
    ...
    test("throws error when URL not found", async () => {
      ...
      await expect(
        UrlService.remove("nonexistent", mockUserId),
      ).rejects.toThrow(ORPCError);
    });
  });
